...
def main():
    ...
    parser.add_option("-r", "--estimate-recovery-parameters", action="store_true", dest="estimate_gamma",
                      default=False, help="Indicates the parameters of the recovery distribution will be estimated")
    ...
    parser.add_option("--threads", action="store", type="int", dest="threads",
    ...
    last_date_on_file = df_full.time.max()
    print("The last date on the data file is %s", last_date_on_file)
    ...
    fname = location + 'posterior_samples_' + today.strftime("%m%d") + '.csv'
    dsaobj.mh_chains.to_csv(os.path.join(plot_folder, fname))
    ...
    fig_rt, rt_trajectories = dsaobj.Rt_plot(samples, dates=dates, theta=dsaobj.theta)
    fname = location+'_Rt_trajectory'+ today.strftime("%m%d")
    fig_save(fig_rt, plot_folder, fname)
    fname = location+'_Rt_trajectory'+ today.strftime("%m%d")+'.csv'
    rt_trajectories.to_csv(os.path.join(plot_folder, fname),index=False)
    ...
    if estimate_gamma:
        ...
        print('Estimate of gamma is\n')
        print(dsaobj.g)
        print('Estimate of delta is\n')
        print(dsaobj.delta)
        if ifMPI:

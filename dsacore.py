...
class DSA():
    ...
    def Rt_plot(self, samples, dates, theta=None):
        nSamples = np.size(samples, axis=0)
        nDays = len(dates)
        time_points = np.arange(nDays)
        r_t = np.zeros((nSamples, nDays), dtype=np.float)
        ...
        for i in range(nSamples):
            ...
            S = interp1d(t, sol)
            r_t[i] = np.asarray(list(r0*S(x) for x in time_points))

        m = np.mean(r_t, axis=0)
        median = np.quantile(r_t, q=0.5, axis=0)
        l = np.quantile(r_t, q=0.025, axis=0)
        h = np.quantile(r_t, q=0.975, axis=0)

        my_plot_configs()
        fig = plt.figure()
        # l2median, = plt.plot(dates['d'].dt.date, median, '-.', color=greys['grey2'].get_rgb(), lw=2, label='Median')
        l2, = plt.plot(dates['d'].dt.date, m, '-', color=cyans['cyan5'].get_rgb(), lw=2, label="Mean")
        ...
        plt.axhline(y=1.0, color=greys['grey4'].get_rgb(), linestyle='dotted')
        plt.xlabel('Dates')
        plt.ylabel('$R_t$')
        # plt.ylim(0, 2000000)
        # plt.legend(handles=[l2mle, l2median, l2])
    ...
    def predict(self, samples, df, dates, n0=1, d0=0, theta=None):
        ...
        # l2median, = plt.plot(dates['d'].dt.date, median, '-.', color=greys['grey2'].get_rgb(), lw=3, label='Median')
...
#                       lw=2, label='Actual')
        ...
        # plt.ylim(0, 2000000)
        # plt.legend(handles=[l2mle, l2median, l2])
        # plt.legend(handles=[l2median, l7])
        plt.legend(handles=[l2])
        ax = plt.gca()
        date_form = DateFormatter("%m-%d")
        ax.xaxis.set_major_formatter(date_form)
        ...
        mean_daily_mean = np.mean(mean_daily, axis=0)
        mean_daily_low = np.quantile(mean_daily, q=0.025, axis=0)
        mean_daily_high = np.quantile(mean_daily, q=0.975, axis=0)
        ...
        # m_daily = np.append(m[1], np.diff(m))
        m_daily = np.append(d0, np.diff(m))
        m_daily_mle = np.append(d0, np.diff(mle))
        m_daily_median = np.append(d0, np.diff(median))
        ...
        l2, = plt.plot(dates['d'].dt.date, m_daily, '-', color=cyans['cyan5'].get_rgb(), lw=3,
                       label='Mean')
        # l2mle, = plt.plot(dates['d'].dt.date, m_daily_mle, '-.', color=greys['grey2'].get_rgb(), lw=3, label='Prediction')
        # l2median, = plt.plot(dates['d'].dt.date, m_daily_median, '-', color=maroons['maroon3'].get_rgb(), lw=3,
        #                   label='Median')

        l5 = plt.fill_between(dates['d'].dt.date, mean_daily_low, mean_daily_high, alpha=.1, color=cyans['cyan1'].get_rgb())
...
#        l6 = plt.axvline(x=df['time'][self.T - 1], color=maroons['maroon3'].get_rgb(), linestyle='-')
        ...
        plt.ylim(0, 25000)
        # plt.legend(handles=[l2mle, l2median, l2])
        # plt.legend(handles=[l2median, l2])
        plt.legend(handles=[l2])
        ax = plt.gca()
        date_form = DateFormatter("%m-%d")
        ax.xaxis.set_major_formatter(date_form)
        sns.despine()
        # plt.legend(handles=[l2mle, l2])
    ...
    def plot_density_fit_posterior(self, samples):
        ...
        fig = plt.figure()
        plt.plot(self.timepoints, Dmean, '-', color=purplybrown['purplybrown4'].get_rgb(), lw=3)
        plt.plot(self.timepoints, Dslow, '--', color=purplybrown['purplybrown4'].get_rgb(), lw=1)
        plt.plot(self.timepoints, Dshigh, '--', color=purplybrown['purplybrown4'].get_rgb(), lw=1)
        ...
        plt.fill_between(self.timepoints, Dslow, Dshigh, alpha=.3, color=bluegreys['bluegrey1'].get_rgb())
        plt.legend()
